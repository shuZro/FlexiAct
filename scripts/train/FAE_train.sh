...
export OUTPUT_PATH="/group/40075/leizizhang/CogVideo-Output/open/${PROJECT_NAME}"
...
CUDA_VISIBLE_DEVICES=$visible_devices accelerate launch --config_file $config_file --machine_rank 0 --num_processes $num_processes --main_process_port $main_process_port \
  ...
  --meta_file_path benchmark/captions/human/${motion_type}/crop.csv \
  --val_meta_file_path benchmark/captions/human/${motion_type}/crop.csv \
  --validating_steps 500 \
  --resume_from_checkpoint latest \
  --proportion_empty_prompts $proportion_empty_prompts \
  --ckpt_path $ckpt_path \
  --max_train_steps 1000000 \
  --val_csv_path benchmark/captions/human/${motion_type}/val_image.csv \
  --stage $stage \
  --rank 64 \
  --lora_alpha 32 \
